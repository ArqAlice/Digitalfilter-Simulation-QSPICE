...
class BQ_filter:
    ...
    def BQfilter_time_domein(self, x):
        y = signal.sosfilt(self.sos, x)
        return y
    
    def BQ_filter_freq_domein(self, fs):
...
y = iir.IIR_filter_time_domain(N_IIR_MULTIPLE, x)
#y = bq.BQfilter_time_domein(x)

# 周波数応答を算出
#w, h1 = fir.FIR_filter_freq_domain(N_FIR_MULTIPLE, FS * OVERSAMPLING_RATIO)
w, h2 = iir.IIR_filter_freq_domain(N_IIR_MULTIPLE, FS * OVERSAMPLING_RATIO)
#w, h3 = bq.BQ_filter_freq_domein(FS * OVERSAMPLING_RATIO)
